...
loc_geom_creator <- function(dirs,lkp_locations){
  ...
  loc_geom <- merge(x=loc_geom, y=lkp_locations, by="location_names")
...
loc_geom_pertype <- function(dirs, datatype, lkp_locations){
  ...
  #print(list_pertype)
  # list.dirs(path=)
  if (length(list_pertype)>0) loc_geom_creator(list_pertype,lkp_locations)
  #print(list_pertype)
}
...
availability_table_simpl <- function(location_name, datatype){
  ...
  #location_name = "0150845E0458190N"
  #datatype="survey"
  ...
  if(dir.exists(root_path_loc_datype)==TRUE){
    ...
    if (datatype %in% c("sensors", "samples", "survey")){#do the following only for csv files (alphanumeric)
      ...
      #Creating a factor with order on the "step column" 
      df$step <- factor(df$step, order=TRUE, levels=c("1s","1","5","10","30","HOUR","DAY","MONTH","SURVEY"))
